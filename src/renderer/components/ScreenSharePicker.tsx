...
if (isWindows) {
    // Tear down any active winaudio capture + AudioContext when our own
    // screenshare ends, so the WASAPI loopback session doesn't leak.
    onceReady.then(() => {
        FluxDispatcher.subscribe("STREAM_CLOSE", ({ streamKey }: { streamKey: string }) => {
            const owner = streamKey.split(":").at(-1);
            ...
            if (session) {
                session.stop().catch(e => logger.warn("[winaudio] stop on STREAM_CLOSE failed:", e));
            }
        });
    });
}
